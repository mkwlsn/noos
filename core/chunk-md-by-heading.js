...
function detectPrimaryHeadingLevel(content) {
  ...
  console.log(`   📊 Heading analysis: ${Object.entries(headingCounts).map(([k,v]) => `${k}:${v}`).join(', ')} → Primary: ${primaryLevel}`);
...
function inferChunkType(title, content) {
  ...
  if (title.toLowerCase().includes('error') || title.includes('HTTP')) return 'error';
...
function mergeSmallChunks(chunks) {
  ...
  while (i < chunks.length) {
    ...
    if (current.content.length < MIN_CHUNK_SIZE && i < chunks.length - 1) {
      ...
      console.log(`   ⚠️ Skipping tiny final chunk: "${current.title}" (${current.content.length} chars)`);
      errorLog.push(`[WARN] Merge process: Skipped tiny final chunk "${current.title}" (${current.content.length} chars)`);
...
function createChunkFrontmatter(chunk, parentFrontmatter, parentSlug, index, totalChunks, allChunkSlugs) {
...
if (!fs.existsSync(cleanDir)) {
  ...
  console.error(`   Run clean-html.js first to generate cleaned markdown files`);
  process.exit(1);
}

const markdownFiles = fs.readdirSync(cleanDir)
...
for (const filename of markdownFiles) {
  ...
  try {
    ...
    if (!content || content.trim().length < MIN_CHUNK_SIZE) {
      ...
      console.log(`   ⚠️ Skipping: insufficient content (${content.length} chars)`);
      errorLog.push(error);
      continue;
    }
    ...
    const validChunks = chunks.filter(chunk => {
      if (chunk.content.length < MIN_CHUNK_SIZE) {
        ...
        console.log(`   ⚠️ Dropping small chunk: "${chunk.title}" (${chunk.content.length} chars)`);
        errorLog.push(error);
        chunkManifest.chunks_skipped++;
        return false;
      }
      if (chunk.content.length > MAX_CHUNK_SIZE) {
        ...
        console.log(`   ⚠️ Large chunk detected: "${chunk.title}" (${chunk.content.length} chars)`);
    ...
    validChunks.forEach((chunk, index) => {
      const chunkFrontmatter = createChunkFrontmatter(
        chunk, frontmatter, parentSlug, index, validChunks.length, allChunkSlugs
      );
      ...
      const frontmatterYaml = Object.entries(chunkFrontmatter)
        .map(([key, value]) => {
          ...
          if (Array.isArray(value)) {
            return value.length === 0 ? `${key}: []` : `${key}: [${value.map(v => `"${v}"`).join(', ')}]`;
          }
          return `${key}: ${value}`;
        })
        .join('\n');
    ...
    console.log(`   ✅ Created ${validChunks.length} chunks (${processingTime}ms)`);
    console.log(`   📊 Tokens: ${fileChunks.reduce((sum, c) => sum + c.token_count, 0)}, Chars: ${fileChunks.reduce((sum, c) => sum + c.char_count, 0)}`);
...
chunkManifest.avg_chunk_size = chunkManifest.chunks_created > 0 ? 
  Math.round(totalChars / chunkManifest.chunks_created) : 0;
...
Object.entries(docTypeBreakdown).forEach(([type, stats]) => {
  console.log(`   ${type}: ${stats.files} files → ${stats.chunks} chunks (${stats.tokens.toLocaleString()} tokens)`);
});
...
const tokenDistribution = chunkManifest.files.flatMap(f => f.chunks).map(c => c.token_count).sort((a,b) => a-b);
...
console.log(`   ⚡ Vector ready: ${chunkManifest.chunks_created} atomic concepts`);
console.log(`   🧠 LLM optimized: ${Math.round(tokenDistribution.filter(t => t <= 2000).length / tokenDistribution.length * 100)}% chunks ≤ 2K tokens`);
console.log(`   🔗 Sibling links: ${chunkManifest.chunks_created * (chunkManifest.chunks_created > 1 ? 1 : 0)} cross-references`);
console.log(`   🏷️ Tags ready: ${chunkManifest.chunks_created} chunks prepared for enrichment`);

console.log("");
console.log("🚀 NEXT STEPS");
console.log("   1. Review sample chunks in chunks/ directory");
console.log("   2. Filter by chunk_type for targeted processing (method/guide/tutorial/error)");
...
if (errorLog.length > 0) {
  console.log("   7. Review chunk-errors.log for quality issues (grep '[ERROR]' for critical)");
}
console.log("");